...
import { NgbModal, NgbModalModule, NgbPopoverModule } from '@ng-bootstrap/ng-bootstrap'
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, NgbModalModule, NgbPopoverModule, RoleUpsertModalComponent],
...
export class RolesManagementComponent implements OnInit {
  ...
  getTruncatedActionNames(actionIds?: number[]): string {
    if (!actionIds || actionIds.length === 0) return '-'

    const maxVisible = 3
    const names = actionIds.slice(0, maxVisible).map((id) => {
      const raw = this.actions.find((a) => a._id === id)?.name || `#${id}`
      return this.toSentenceCase(raw)
    })

    if (actionIds.length <= maxVisible) {
      return names.join(', ')
    }

    return names.join(', ') + ` +${actionIds.length - maxVisible} more`
  }

  getAllActionNames(actionIds?: number[]): string {
    if (!actionIds || actionIds.length === 0) return '-'
  ...
  getAllActionsArray(actionIds?: number[]): string[] {
    if (!actionIds || actionIds.length === 0) return ['-']
  ...
  onEditRole(role: AppRole) {
    if (!this.canEditRole) {
      this.notifyPermissionDenied('Unauthorized action: Edit Role.')
      return
    }
  ...
  async onDeleteRole(role: AppRole) {
    if (!this.canDeleteRole) {
      this.notifyPermissionDenied('Unauthorized action: Delete Role.')
      return
    }
