...
export class TestCasesHomeComponent {
  ...
  getCompletionLabel(planId: string): 'Valid' | 'Unvalid' {
    return this.getPlanStatus(planId) === 'completed' ? 'Valid' : 'Unvalid'
  }

  getSuiteCardStatus(suite: TestSuiteDto): 'Valid' | 'Unvalid' {
    return suite?.sessionStatus === 'complete' ? 'Valid' : 'Unvalid'
  }
